...
public class MASISMLSMPClient {
     ...
     * Logger.
     */
    private final static Logger LOGGER = LoggerFactory.getLogger(MASISMLSMPClient.class);
    ...
    /**
     * TODO: to be changed
     * This is the default prefix. It is PEPPOL.
     */
    ...
    /**
     * This is the SML domain. It tells which DNS to use, that will have the NAPTR.
     */
    private String smlDomain;

    /**
     * This is the SML process identifier. It is a field, since it is shared by the methods.
     */
    ...
    public MASISMLSMPClient() {
        try {
            LOGGER.debug("In SMPSMLClient constructor");
            md = MessageDigest.getInstance("SHA-256");

        } catch (NoSuchAlgorithmException e) {
            LOGGER.error("Uh, message digest not found?", e);
     ...
     *
     * @param countryCode     The two digits code of the country which we're searching for
     *                        capabilities. Example: "pt", "at".
     * @param epsosAction     The action for which we need to obtain capabilities. For
     *                        instance, "epSOS-21"
     * @param verifySignature true if all the signatures must be validated.
     * @throws SMLSMPClientException If the lookup is wrong.
     */
    public void lookup(String countryCode, String epsosAction, boolean verifySignature) throws SMLSMPClientException {
        ...
        try {
            ...
            LOGGER.debug("SML begins ...");
            String hostToLookup = prepareSmlQuery(countryCode);
            String serviceGroupUrl = doDNSQuery(hostToLookup);
            long end = System.currentTimeMillis();
            long total = end - start;
            LOGGER.debug("SML ends. Took '{}' msec", total);
            LOGGER.debug("SMP begins");
            start = System.currentTimeMillis();
            URL signedInfoURL = getServiceGroup(serviceGroupUrl, epsosAction);
            getSignedServiceInformation(signedInfoURL);
            end = System.currentTimeMillis();
            total = end - start;
            LOGGER.debug("SMP ends. Took '{}' msec", total);
        } catch (TransformerException | UnsupportedEncodingException | DecoderException | TextParseException
                | EncoderException e) {
            LOGGER.error("Should never been thrown. We're OpenNCP, we don't run in a device" + e);
            throw new SMLSMPClientException(e);
        } catch (IOException e) {
            LOGGER.error("I/O Exception. The SMP SML service is down: " + e.getMessage(), e);
            throw new SMLSMPClientException(e);
        } catch (CertificateException e) {
            LOGGER.error("A certificate exception occurred. The certificate for the party " + countryCode + " is corrupted",
                    e);
            throw new SMLSMPClientException(e);
        }
    }
     ...
     *
     * @param serviceGroupUrl The url of the service group.
     * @param epSOSaction     the epsos action for which we search data.
     * @return The signed service information url.
     * @throws IOException           If the connection is down!
     * @throws DecoderException      If the URL is not properl formed
     * @throws SMLSMPClientException If no servicegroup can't be found
     */
    private URL getServiceGroup(String serviceGroupUrl, String epSOSaction)
        ...
        LOGGER.debug("Creating the epSOS action");
...
//		String finalDocumentIdentifier = SMP_PREFIX + ep1.toLowerCase();
        ...
        LOGGER.debug("Found documentIdentifier '{}'", finalDocumentIdentifier);
        ...
        LOGGER.debug("Found documentIdentifier '{}'", finalDocumentIdentifier);
        LOGGER.debug("Final document identifier is: '{}'", finalDocumentIdentifier);
        LOGGER.debug("Obtaining the Service Group");
    ...
    /**
     * TODO: here we can improve a lot
     * Obtain a Document from HTTP. This code is coming from an example of Apache httpclient.
     *
     * @param url The url of the DOM.
     * @return the Document returned from the GET.
     * @throws IOException
     */
    private Document getFromHTTP(String url) throws IOException {

        LOGGER.debug("Doing HTTP to '{}'", url);
        try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
            ...
            try {
                ...
                LOGGER.error("Unable to get the SMP record", e.getMessage(), e);
                throw e;
            }

            try {
                LOGGER.debug("HTTP Response: " + response1.getStatusLine());
                HttpEntity entity1 = response1.getEntity();

                LOGGER.debug("Building the document");
     ...
     * the service group.
     *
     * @param hostToLookup The hostname ready for SML.
     * @return The URL of the service group.
     * @throws EncoderException   Should never been thrown.
     * @throws TextParseException Should never been thrown.
     */
    private String doDNSQuery(String hostToLookup) throws EncoderException, TextParseException {
        LOGGER.debug("In lookup. Using ONLY configured DNS. Looking up: '{}'", hostToLookup);
        Lookup l = new Lookup(hostToLookup, Type.NAPTR);

        LOGGER.debug("Executing ...");
        final Record[] records = l.run();
        if (l.getResult() == Lookup.SUCCESSFUL) {
            LOGGER.debug("Succesfull DNS query, looking if there are some results");
            int size = records.length;
            LOGGER.debug("Found '{}' records. It should be just 1", size);
            if (size != 1) {
            ...
            LOGGER.debug("Obtained RAW data: '{}'", t);
            /*
            ...
            LOGGER.debug("Buggy things happen here... ");
            String[] t1 = t.split(" ");
            ...
            LOGGER.debug("The full record is: '{}'", url1);
            /*
            ...
            LOGGER.debug("Calculated Service Group Name: '{}'", sgName);
            return sgName;

        } else {
            LOGGER.error("DNS query failed: " + l.getErrorString());
     ...
     *
     * @param domain The DNS domain. It MUST be preceded with a dot ".". Example
    ...
    private String prepareSmlQuery(String countryCode) throws UnsupportedEncodingException {
        ...
        LOGGER.debug("Encoding base32");
        String value = BaseEncoding.base32().encode(back) + smlDomain;

        LOGGER.debug("Removing trailing =");
        value = value.replace("=", "");

        LOGGER.debug("Applied the record according with the specifications. Host evaluated is: '{}'", value);
     ...
     *
     * @param url the url used to fetch the signedServiceInformation
     * @throws IOException           In case the SMP service is down.
     * @throws TransformerException  Only if debug is true
     * @throws SMLSMPClientException If the file is malformed
     * @throws CertificateException  if the certificate is not encoded correctly
     */
    public void getSignedServiceInformation(URL url)
        ...
        // Apply Joao fix because of the fact that the SMP record is buggy since it is modified by the
        // SMP SmpXslt.removeSmpSignature(beforeTheFix);
        //
        ...
        if (LOGGER.isDebugEnabled()) {
            ...
            LOGGER.debug("Received: '{}'", new String(baos.toByteArray()));
        }
            ...
            if (n.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (StringUtils.equals(localName, "ServiceEndpointList")) {
                    ...
                    for (int j = 0; j < size1; j++) {
                        ...
                        if (localName11 != null && StringUtils.equals(localName11, "Endpoint")) {
