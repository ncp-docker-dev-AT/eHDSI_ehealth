...
For each eP and each PS there MUST be two versions with the same file name, but with different extensions: XML and PDF.
...
A limitation: the first element of the XML document must be ClinicalDocument. Remove any references to stylesheets etc.
<?xml version="1.0" encoding="UTF-8"?> is naturally ok.
