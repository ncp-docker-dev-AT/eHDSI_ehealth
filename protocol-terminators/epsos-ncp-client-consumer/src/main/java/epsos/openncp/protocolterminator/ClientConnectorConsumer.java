	...
	public List<EpsosDocument1> queryDocuments(Assertion idAssertion, Assertion trcAssertion, String countryCode,
		...
		try {
			...
			try {
				...
				logger.info("request is  {} .{}", request, lineSeparator);
				logger.info("\n");
				logger.info("response is  {} .{}", response, lineSeparator);
				logger.info("\n");
			} catch (Exception e) {
				logger.info("Attempt to display the anvelope failed");
			}
...
//        stub._getServiceClient().addHeader(to);
	...
	public EpsosDocument1 retrieveDocument(Assertion idAssertion, Assertion trcAssertion, String countryCode,
		...
		try {
			...
			RetrieveDocumentResponseDocument retrieveDocumentResponseDocument = clientConnectorStub
					.retrieveDocument(retrieveDocumentDocument);
	...
	/**
	 * @deprecated Method has been deprecated in favor of the implementation of
	 *             retrieveDocument() with language parameter.
	 */
	@Deprecated
	public EpsosDocument1 retrieveDocument(Assertion idAssertion, Assertion trcAssertion, String countryCode,
	 ...
	 *
	 * @return Initialized ClientConnectorServiceStub set to the configured EPR and
	 *         the SOAP version.
	 */
	private ClientConnectorServiceStub initializeServiceStub() {
		...
		try {
			...
			clientConnectorStub._getServiceClient().getOptions()
	...
	/**
	 * Trims the Patient Demographics sent by the client and received by the Client
	 * Connector.
	 *
	 * @param patientDemographics Identity Traits to be trimmed and provided by the
	 *                            by the client
	 */
	private void trimPatientDemographics(PatientDemographics patientDemographics) {
		...
		if (StringUtils.isNotBlank(patientDemographics.getAdministrativeGender())) {
			patientDemographics
					.setAdministrativeGender(StringUtils.trim(patientDemographics.getAdministrativeGender()));
		}
